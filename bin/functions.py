...
def fit_reml(
    ...
    :param p: degree of bases
    :param q: order of penalty
    :param num: num of observations (TODO 6/21: verify)
...
def Bspline_Basis(p, i, u, U):
    ...
    n = m - p - 1
    if (u == U[0] and i == 0) or (
            u == U[m] and i == n):  ## tackling at the boundary
        N = 1
        return N
    ...
    for deg in range(1, p + 1):
        for i1 in range(i, reduce_len):
            if N1[deg - 1, i1] != 0 and N1[deg - 1, i1 + 1] != 0:
                N1[deg, i1] = ((u - U[i1]) / (U[i1 + deg] - U[i1])) * N1[
                    deg - 1, i1] + ((U[i1 + deg + 1] - u) / (
                        U[i1 + deg + 1] - U[i1 + 1])) * N1[deg - 1, i1 + 1]
...
def Bspline_Basis_temp(p, i, u, U, row):
    ...
    for deg in range(1, p + 1
                     ):  # as bottom row is degree 0 and top row is degree p (bottom row already alloted)
        for i1 in range(i, reduce_len):
            if N1[deg - 1, i1] != 0 and N1[deg - 1, i1 + 1] != 0:
                N1[deg, i1] = ((u - U[i1]) / (U[i1 + deg] - U[i1])) * N1[
                    deg - 1, i1] + ((U[i1 + deg + 1] - u) / (
    ...
    ret = np.array(N1[row, i:i + k + 1]).reshape(1, k + 1)
...
def Derivative_bspline_basis(i, p, k, u, U):
    ...
    for x in range(p - k + 1, p + 1):
        for y in range(i, term):
            ...
            if term1 != 0 and term2 != 0:
                N_der[xcoor, ycoor] = x * (term1 / (U[y + x] - U[y])) - x * (
                        term2 / (U[y + x + 1] - U[y + 1]))
...
def XZsigma(B, P, q):
    ...
    C = np.zeros([X.shape[0], X.shape[1] + Z.shape[1]])
    C[:, 0:X.shape[1]] = X
    C[:, X.shape[1]:] = Z
...
def Var_bounds(Data, B, B_dat, theta, P, lamb, confidence=0.95):
    ...
    sigmasq = sigmasq[0][0]
    std = np.sqrt(
        np.diag(sigmasq * B.dot(inv(B_dat.T.dot(B_dat) + P)).dot(B.T))
        )
...
def full_search_nk(Data, p, q):
    ...
    theta = np.linalg.solve(B_dat.T.dot(B_dat) + P,
...
def REML(par, Data, X, Z, sigma):
    ...
    R = sig * np.eye(Data.shape[0])

    V = Z.dot(G).dot(Z.T) + R
    y = Data[:, 1].reshape(-1, 1)
    t11 = np.log(det(V))
    t12 = (y.T.dot(inv(V))).dot(
        np.eye(Data.shape[0]) - X.dot(inv(X.T.dot(inv(V)).dot(X))).dot(
            X.T.dot(inv(V))
        )
    ).dot(y)
    t1 = -0.5 * (t11 + t12)
    t2 = (-Data.shape[0] / 2) * np.log(2 * np.pi)
    t3 = -0.5 * np.log(det(X.T.dot(inv(V)).dot(X)))

    reml = -(
            t1 + t2 + t3)  ## extra minus will correspond to minimization of the objective
...
def max_reml(par, Data, X, Z, sigma):
    ...
    bnds = np.array([(np.e - 2, None), (np.e - 2, None)])
...
def Inference(Data, Cpred, C, lamb, sig, D, confidence=0.95):
    ...
    df_res = Data.shape[0] - 2 * np.trace(Slam) + np.trace(Slam.dot(Slam.T))
    se = np.sqrt(np.diag(sig * Cpred.dot(term).dot(Cpred.T)))
...
def Polynomials_fit(Data, points):
    ...
    y = np.array(Data[:, 1]).reshape(vander.shape[0], 1)
    param = np.array(np.linalg.lstsq(vander, y, rcond=None)[0])
    ...
    vandp = np.array(vandp)

    prediction = vandp.dot(param)
    p = np.zeros([prediction.shape[0], 2])
    p[:, 0] = points.flatten()
...
def Outlier(Data, thresh1, thresh2):
    ...
    r_long = np.zeros([Data.shape[0], 2])
    r_long[:, 0] = Data[:, 0]
...
#     ax = plt.subplot2grid((1, 1), (0, 0), fig=f1)
#
#     ax, form = conf_reml(ax, data)
